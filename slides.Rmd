...
![](img/Paddock28_21.2.17.jpg){height=75mm}
...
\source{Braconid wasp \emph{Microctonus hyperodae} by Mark McNeill, \href{https://creativecommons.org/licenses/by/3.0/us/}{CC-BY-3.0}.}
...
<!-- # Pivot to insect genomics \scalerel{\includegraphics{img/backhand-index-pointing-down_medium-skin-tone_1f447-1f3fd_1f3fd.png}}{X} -->


# \emoji1{} Check out these genomes

Does it work in normal \emoji1{} text?

<!-- # Pivot to insect genomics  -->
...
```{r frac_discarded, fig.width = fw * 0.5}
geo_only <- readRDS("data/frac_discarded.geo.Rds")
geo_only[, pop2 := factor(pop, levels = rev(pop_order))]
# plot
ggplot(geo_only[!is.na(pop2)],
       aes(x = pop2,
           y = frac_discarded * 100,
           colour = pop2)) +
    coord_flip() +
    xlab(NULL) + ylab("Bases discarded (%)") +
...
```{r flagstats, fig.width = fw * 0.5}

flagstats <- readRDS("data/flagstats.Rds")
ggplot(flagstats[!is.na(pop2)][
    category == "mapped %"],
    aes(x = pop2,
        y = qc_pass,
        colour = pop2)) +
    coord_flip() +
    xlab(NULL) + ylab("Mapped (%)") +
    scale_colour_viridis_d(guide = FALSE) +
    geom_boxplot(fill = NA,
                 colour = alpha("black", 0.5),
                 outlier.colour = NA,
...
```{r mean_dp, fig.width = fw * 0.5}
mean_dp <- readRDS("data/mean_dp.Rds")
ggplot(mean_dp[!is.na(pop2)],
       aes(x = pop2,
           y = mean_dp,
           colour = pop2)) +
    coord_flip() +
    xlab(NULL) + ylab("Mean depth across loci") +
    scale_colour_viridis_d(guide = FALSE) +
    geom_boxplot(fill = NA,
                 colour = alpha("black", 0.5),
                 outlier.colour = NA,
...
# Genetic differentiation between parasitised an unparasitised weevils
...
![](/home/tom/Projects/microctonus-compare/wga/030_wga/wga.pdf)
